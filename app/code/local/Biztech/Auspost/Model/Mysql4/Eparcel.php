...
class Biztech_Auspost_Model_Mysql4_Eparcel extends Mage_Core_Model_Mysql4_Abstract
{
    const MIN_CSV_COLUMN_COUNT = 8;

    protected function _construct()
    {
        $this->_init('auspost/eparcel', 'pk');
    }
            ...
            if (!empty($row) && ($j < 5)) {
                ...
                foreach ($row as $data) {
                    try {
                        ...
                        if(Mage::getStoreConfig('carriers/eparcel/weight_unit')=="gm"){
                            $price += ((float)($data['price_per_kg']) * $conditionValue)/1000;
                        }else{
    ...
    public function uploadAndImport(Varien_Object $object)
    {
        ...
        if (!empty($csvFile)) {
            ...
            $table = Mage::getSingleton('core/resource')->getTableName('auspost/eparcel');

            $websiteId = $object->getScopeId();
            
            if (isset($_POST['groups']['eparcel']['fields']['condition_name']['inherit'])) {
                //$conditionName = (string)Mage::getConfig()->getNode('carriers/eparcel/condition_name');
                $conditionName = (string)Mage::getStoreConfig('carriers/eparcel/condition_name');
            } else {
                $conditionName = $_POST['groups']['eparcel']['fields']['condition_name']['value'];
            }

            $conditionFullName = Mage::getModel('auspost/carrier_eparcel')->getCode('condition_name_short', $conditionName);
