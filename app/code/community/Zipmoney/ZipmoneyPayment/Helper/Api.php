...
class Zipmoney_ZipmoneyPayment_Helper_Api extends Mage_Core_Helper_Abstract
{
    ...
    protected function _getProductImage($oProduct)
    {
        $oLogging = Mage::helper('zipmoneypayment/logging');
        $vImageUrl = '';
        try {
            $vImageUrl = (string)Mage::helper('catalog/image')->init($oProduct, 'thumbnail');
        } catch (Exception $e) {
            $oLogging->writeLog($this->__('An error occurred during getting item image for product ' . $oProduct->getId() . '.'), Zend_Log::WARN);
            $oLogging->writeLog($e->getMessage(), Zend_Log::ERR);
            $oLogging->writeLog($e->getTraceAsString(), Zend_Log::DEBUG);
        }
        return $vImageUrl;
    ...
    public function getProductShortDescription($oProduct, $iStoreId)
    {
        $vDescription = $oProduct->getShortDescription();
        if (!$vDescription) {
            $vDescription = $oProduct->getResource()->getAttributeRawValue($oProduct->getId(), 'short_description', $iStoreId);
        }
        return $vDescription;
    }
    ...
    public function getOrderDataByQuote(Mage_Sales_Model_Quote $oQuote, $oShippingAddress = null)
    {
        ...
        foreach($oQuote->getAllItems() as $oItem) {
            ...
            if ($oItem->getDescription()) {
                ...
                $vDescription = $this->getProductShortDescription($oItem->getProduct(), $oQuote->getStoreId());
            }
            ...
            $aItem['image_url']     = $this->_getProductImage($oItem->getProduct());
    ...
    public function getOrderDataByOrder(Mage_Sales_Model_Order $oOrder, $bStatusState = false)
    {
        ...
        foreach($oOrder->getAllItems() as $oItem) {
            ...
            if ($oItem->getDescription()) {
                ...
                $vDescription = $this->getProductShortDescription($oItem->getProduct(), $oOrder->getStoreId());
            }
            $aItem['id']            = $oItem->getId();
            $aItem['name']          = $oItem->getName();
            $aItem['sku']           = $oItem->getSku();
