...
class Mage_Sales_Model_Order_Config extends Mage_Core_Model_Config_Base
{
    ...
    public function getStatuses()
    {
        $statuses = Mage::getResourceModel('sales/order_status_collection')
            ->toOptionHash();
             //remove some status order didn't use.
             unset($statuses['paypal_reversed']); // add this line in your 
             unset($statuses['paypal_canceled_reversal']);
             unset($statuses['pending_paypal']);
             unset($statuses['zip_pending']); 
             unset($statuses['zip_order_cancelled']);
             unset($statuses['zip_declined']);
             unset($statuses['zip_capture_pending']);
             unset($statuses['zip_captured']);
             unset($statuses['zip_cancelled']);
            //  unset($statuses['processing']);
             unset($statuses['zip_authorise_under_review']);
             unset($statuses['zip_authorised']);
             unset($statuses['closed']);
             unset($statuses['paid']);
             unset($statuses['payment_review']);
             unset($statuses['zip_refund_pending']);
             unset($statuses['zip_refund']);
